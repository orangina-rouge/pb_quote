...
//             PrestaShopLogger::addLog("Searching room");
            $room = 'Divers';
//             foreach ($order_detail['customizedDatas'] as $customizationPerAddress) {
//                 foreach ($customizationPerAddress as $customizationId => $customization) {
//                     if (isset($customization['datas'][1]) && count($customization['datas'][1]) > 0) {
//                         foreach ($customization['datas'][1] as $customization_infos_id => $customization_infos) {
//                             if ($customization_infos['name'] == 'tva') {
//                                 $room = $customization_infos['value'];
//                                 unset($customization['datas'][1][$customization_infos_id]);
//                             }
//                         }
//                     }
//                 }
...
//             PrestaShopLogger::addLog("Found room '$room'");
            if(!isset($order_details_per_room[$room])) {
                $order_details_per_room[$room] = array();
            }
            $order_details_per_room[$room][] = $order_detail;
        }
        unset(
            $tax_temp,
            $order_detail
        ...
        $taxTabContent = $this->getTaxTabContent();
        $data = array(
            'order' => $this->order,
            'order_invoice' => $this->order_invoice,
            'order_details' => $order_details,
            'order_details_per_room' => $order_details_per_room,
            'carrier' => $carrier,
        ...
        $tpls = array(
            'style_tab' => $this->smarty->fetch($this->getTemplate('quote.style-tab')),
            'addresses_tab' => $this->smarty->fetch($this->getTemplate('quote.addresses-tab')),
            'summary_tab' => $this->smarty->fetch($this->getTemplate('quote.summary-tab')),
            'product_tab' => $this->smarty->fetch($this->getTemplate('quote.product-tab')),
            'tax_tab' => $taxTabContent,
            'payment_tab' => $this->smarty->fetch($this->getTemplate('quote.payment-tab')),
            'note_tab' => $this->smarty->fetch($this->getTemplate('quote.note-tab')),
            'total_tab' => $this->smarty->fetch($this->getTemplate('quote.total-tab')),
            'shipping_tab' => $this->smarty->fetch($this->getTemplate('quote.shipping-tab')),
        );
    ...
    public function getTaxTabContent()
    {
        ...
        return $this->smarty->fetch($this->getTemplate('quote.tax-tab'));
    }
    ...
    */
    ...
    protected function getTemplate($template_name)
    {
        ...
        $overridden_template = _PS_CORE_DIR_ . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'pb_quote' . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR . $template_name . '.tpl';
        if (file_exists($overridden_template)) {
            $template = $overridden_template;
        } elseif (file_exists($default_template)) {
            $template = $default_template;
        }
//         PrestaShopLogger::addLog("Template found $template");
        return $template;
    }
}
